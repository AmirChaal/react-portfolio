...
export default function AvyHeadFace({ avyHeadGroup, visible, ...wrapperProps }: { avyHeadGroup: Group | null, visible: boolean } & ComponentPropsWithoutRef<"group">) {
   ...
   useFrame((state, delta) => {
      const safeDelta = Math.min(delta, 0.05); // cap at ~50ms (20 FPS)
      let cursor3D
      ...
      if (leftEyeRef.current && rightEyeRef.current && faceGroupRef.current && faceGroupRef.current.parent) {
         ...
         if (eyesOpen) {
            leftEyeRef.current.rotation.z += (eyeRotation - leftEyeRef.current.rotation.z) * smoothing * safeDelta * 25;
            rightEyeRef.current.rotation.z += (eyeRotation - rightEyeRef.current.rotation.z) * smoothing * safeDelta * 25;
         } else {
